...
export const AuthProvider = ({ children }) => {
  ...
  const [token, setToken] = useState(localStorage.getItem('drivexa_rider_token'));
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      localStorage.setItem('drivexa_rider_token', receivedToken);
  ...
  const signup = async (userData) => {
    setLoading(true);
    try {
      const response = await api.post('/auth/rider/signup', userData);
      const { token: receivedToken, user: loggedUser } = response.data;
      localStorage.setItem('drivexa_rider_token', receivedToken);
      setToken(receivedToken);
      setUser(loggedUser);
  ...
  const logout = () => {
    localStorage.removeItem('drivexa_rider_token');
    setToken(null);
    setUser(null);
  };
